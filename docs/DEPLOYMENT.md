...
### Remote world & config overlay

The demo instance can fetch its world content, sprite definitions, achievements,
and config overlay from a separate "lore repo" hosted on Cloudflare
...
|------|------|-----------------------------------------------|-------|
| `LORE_CONFIG_URL` | secret | `https://myURL/config/application-local.yaml` | Base lore config overlay |
| `WORLD_ZONES_BASE_URL` | secret | `https://myURL/world`                         | **No trailing slash** — R2 treats `//` as a distinct key and returns 404 |
| `SPRITES_URL` | secret | `https://myURL/sprites.yaml`                  | Arcanum-generated sprite definitions |
| `ACHIEVEMENTS_URL` | secret | `https://myURL/world/achievements.yaml`       | Achievement definitions |
| `HOSTNAME` | variable | `myURL`                                       | Required for nginx + TLS install — the whole nginx block is gated on this being non-empty |
