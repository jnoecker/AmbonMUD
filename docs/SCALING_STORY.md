...
3. ~~**Telnet transport thread pressure**~~: resolved — `BlockingSocketTransport` now uses JDK 21 virtual threads (PR #313), eliminating platform-thread overhead for concurrent telnet connections. WebSocket transport was already non-blocking via Ktor/Netty.
...
2. *(done)* ~~**Virtual threads for telnet transport (#301)**~~: `BlockingSocketTransport` now uses JDK 21 virtual threads (PR #313). Thousands of concurrent telnet sessions without thread pool pressure.
