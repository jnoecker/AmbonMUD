...
| **AkathavaeSystem** | `engine/AkathavaeSystem.kt` | Illumination + pacifist explorer path: **anyone** can `illuminate` to fill a personal Arcanum (unpledged odds/XP scaled by `unpledgedSuccessMultiplier`/`unpledgedXpMultiplier`), but the pledge is the mastery tier — `pledge` at a shrine (`akathavaeShrine: true` room flag) switches the player into the AKATHAVAE class (the game's only active-class switch — former class stashed in `preAkathavaeClass`, restored on renounce; vitals rescale via `recomputeVitalCaps`; multiclassing locked while pledged). Pledged-only depth: drop cataloguing, quest kill credit + rubbings, passive room/item discovery, conjured `wardrobe` equipment, and sealing illumination world-firsts ("First illuminated by …"). A separate "First slain by …" world-first is stamped on any player's killing blow (`CombatSystemCallbacks.onMobSlain` → `slain:<templateKey>` in the world-state snapshot). `Arcanum.*` GMCP |
