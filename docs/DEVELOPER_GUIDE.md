...
| **AkathavaeSystem** | `engine/AkathavaeSystem.kt` | Pacifist explorer path: `pledge` at a shrine (`akathavaeShrine: true` room flag) switches the player into the AKATHAVAE class (the game's only active-class switch — former class stashed in `preAkathavaeClass`, restored on renounce; vitals rescale via `recomputeVitalCaps`; multiclassing locked while pledged); `illuminate` records creatures stat-driven (kill-equivalent on success: drops, gold, quest credit, XP), rooms/items discovered passively, permanent world-firsts ("First illuminated by …") in the world-state snapshot, conjured `wardrobe` equipment, `Arcanum.*` GMCP |
