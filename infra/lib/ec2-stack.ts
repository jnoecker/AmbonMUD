    ...
    userData.addCommands(
      ...
      ...(postgresSidecar
        ? [
            ...
            '  # Subshell scopes umask 077 so it does not leak into the rest of',
            '  # user-data. Without the subshell every later mkdir/write (including',
            '  # /app/prometheus and /app/grafana) inherited 700/600 modes, which',
            '  # broke both containers: their processes run as non-root UIDs (472 /',
            '  # nobody) and mount those dirs :ro, so root-owned 700 was unreadable.',
            '  (',
            '    umask 077',
