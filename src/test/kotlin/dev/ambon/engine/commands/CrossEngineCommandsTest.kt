...
class CrossEngineCommandsTest {
    ...
    @Test
    fun `gossip broadcasts GlobalBroadcast via inter-engine bus`() = runTest {
        val sid = SessionId(1)
    ...
    @Test
    fun `tell to unknown player sends TellMessage via bus`() = runTest {
        val sid = SessionId(1)
        login(players, sid, "Alice")
        drain(outbound)
    ...
    @Test
    fun `tell to unknown player without bus shows error`() = runTest {
        // Router without bus
        val noBusRouter = CommandRouter(
            world = world,
            players = players,
            mobs = mobs,
            items = items,
    ...
    @Test
    fun `kick unknown player sends KickRequest via bus`() = runTest {
        val sid = SessionId(1)
        login(players, sid, "Alice")
    ...
    @Test
    fun `shutdown broadcasts GlobalBroadcast SHUTDOWN via bus`() = runTest {
        val sid = SessionId(1)
        ...
        val shutdownRouter = CommandRouter(
            world = world,
            players = players,
            mobs = mobs,
            items = items,
    ...
    @Test
    fun `transfer unknown player sends TransferRequest via bus`() = runTest {
        val sid = SessionId(1)
        login(players, sid, "Admin")
        players.get(sid)!!.isStaff = true
        drain(outbound)
    ...
    private suspend fun login(players: PlayerRegistry, sessionId: SessionId, name: String) {
