...
class GrpcInboundBusTest {
    ...
    fun setUp() {
        ...
        bus = GrpcInboundBus(delegate = delegate, grpcSendChannel = grpcChannel, scope = scope)
    }
    ...
    fun `send forwards event to gRPC channel`() =
        runBlocking {
            ...
            bus.send(event)
            delay(30) // allow fire-and-forget coroutine to complete
    ...
    @Test
    fun `trySend forwards event to gRPC channel on success`() =
        runBlocking {
    ...
    @Test
    fun `send also places event in delegate for local tryReceive`() =
        runBlocking {
