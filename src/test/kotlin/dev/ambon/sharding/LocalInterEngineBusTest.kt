...
class LocalInterEngineBusTest {
    @Test
    fun `broadcast delivers message to incoming channel`() =
        runBlocking {
    ...
    @Test
    fun `sendTo delivers message to incoming channel`() =
        runBlocking {
            val bus = LocalInterEngineBus()
            bus.start()
    ...
    @Test
    fun `close causes incoming to be closed`() =
        runBlocking {
            val bus = LocalInterEngineBus()
            bus.start()
            bus.close()
    ...
    @Test
    fun `multiple messages are queued in order`() =
        runBlocking {
            val bus = LocalInterEngineBus()
            bus.start()
