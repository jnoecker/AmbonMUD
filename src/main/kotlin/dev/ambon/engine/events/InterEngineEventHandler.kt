...
class InterEngineEventHandler(
    ...
    /** Broadcasts a who-request to peer engines and registers a pending response. */
    internal suspend fun handleRemoteWho(sessionId: SessionId) {
    ...
    /** Sends any collected who-responses whose deadline has elapsed. Called from the tick loop. */
    internal suspend fun flushDueWhoResponses(nowEpochMs: Long = clock.millis()) {
    ...
    /** Removes all pending who-requests associated with [sessionId] (called on disconnect). */
    internal fun removePendingWhoRequestsFor(sessionId: SessionId) {
        val itr = pendingWhoRequests.iterator()
        while (itr.hasNext()) {
            if (itr.next().value.sessionId == sessionId) itr.remove()
        }
    }

    private fun onWhoResponse(msg: InterEngineMessage.WhoResponse) {
