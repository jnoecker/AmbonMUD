...
object PlayersTable : Table("players") {
     ...
     *
     * Pass `includeId = false` when inserting into an auto-increment table where
     * the id should be generated by the database; the default `true` matches
     * upsert/save semantics where the caller has an authoritative id.
     */
    fun writeRecord(statement: UpdateBuilder<*>, record: PlayerRecord, includeId: Boolean = true) {
        if (includeId) statement[id] = record.id.value
